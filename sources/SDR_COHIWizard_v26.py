...
############################## GENERAL MENU FUNCTIONS  ####################################
    ...
    def FileOpen(self):   #TODO: shift to general system module ?
        ...
        if not self.m["f1"]:
            #KIPP: sys_state.set_status(self.m)
            return False
...
if __name__ == '__main__':
    ...
    xcore_v = core_v(xcore_c,xcore_m) # self.ui wird in xcore_v gestartet 
...
#    app.aboutToQuit.connect(win.stop_worker)    #graceful thread termination on app exit
    ...
    if 'resampler_module_v5' in sys.modules:
        ...
        resample_v = rsmp.resample_v(xcore_v.ui,resample_c, resample_m) #TODO: replace sys_state
        ...
        xcore_v.ui.tabWidget.setTabVisible('tab_resample',False)

    if 'view_spectra' in sys.modules:
        view_spectra_m = vsp.view_spectra_m()
        view_spectra_c = vsp.view_spectra_c(view_spectra_m)
        view_spectra_v = vsp.view_spectra_v(xcore_v.ui,view_spectra_c,view_spectra_m)
    ...
    if 'annotate' in sys.modules:
        ...
        annotate_v = ann.annotate_v(xcore_v.ui,annotate_c,annotate_m)
    ...
    if 'yaml_editor' in sys.modules:
        ...
        yamleditor_v = yed.yamleditor_v(xcore_v.ui,yamleditor_c,yamleditor_m)
        tab_dict["list"].append("yamleditor")
    else:
        page = xcore_v.ui.tabWidget.findChild(QWidget, "tab_yamleditor")
        c_index = xcore_v.ui.tabWidget.indexOf(page)
        xcore_v.ui.tabWidget.setTabVisible(c_index,False)

    if 'waveditor' in sys.modules:
        waveditor_m = waved.waveditor_m()
        waveditor_c = waved.waveditor_c(waveditor_m)
        waveditor_v = waved.waveditor_v(xcore_v.ui,waveditor_c,waveditor_m)
        tab_dict["list"].append("waveditor")
    else:
        page = xcore_v.ui.tabWidget.findChild(QWidget, "tab_waveditor")
        c_index = xcore_v.ui.tabWidget.indexOf(page)
        xcore_v.ui.tabWidget.setTabVisible(c_index,False)
    ...
    if 'configuration' in sys.modules:
        ...
        configuration_v = conf.configuration_v(xcore_v.ui,configuration_c,configuration_m)
    else:
        page = xcore_v.ui.tabWidget.findChild(QWidget, "tab_configuration")
        c_index = xcore_v.ui.tabWidget.indexOf(page)
        xcore_v.ui.tabWidget.setTabVisible(c_index,False)
        pass
    ...
    if 'playrec' in sys.modules: #and win.OLD is False:
        ...
        playrec_v = playrec.playrec_v(xcore_v.ui,playrec_c,playrec_m)
    ...
    xcore_v.SigRelay.emit("cm_all_",["QTMAINWINDOWparent",xcore_v])
    sys.exit(app.exec_())
